...
parser.add_argument('--iters_SD', type=int, default=1500, help="iteration for SD activation")
...
parser.add_argument('--control', action='store_true',
                    help="whether use controlnet or only stable diffusion")
...
parser.add_argument('--use_plus', type=bool, default=False,
                    help="IP-adapter version choose")
...
parser.add_argument('--att_scale', type=float, default=0.5,
                    help="scale for cross attention")
...
#     print(opt.image)
    ...
    if opt.six_views:
        ...
        for idx, guidance_type in enumerate(opt.guidance):
            ...
            if 'SD' == guidance_type:
                from guidance.sd_utils import StableDiffusion, StableDiffusion_control, token_replace
#                 print(opt.control)
                if opt.control == True:
                    print('loaded controlnet')
                    guidance['SD'] = StableDiffusion_control(device, opt.fp16, opt.vram_O, opt.sd_version, opt.hf_key, opt.t_range, learned_embeds_path=opt.learned_embeds_path, scale=opt.att_scale, use_plus=opt.use_plus, use_clip=opt.use_clip, clip_t=opt.clip_t, clip_iterative=opt.clip_iterative, clip_version=opt.clip_version, image=opt.image)
                else:
                    print('loaded diffusion')
                    guidance['SD'] = StableDiffusion(device, opt.fp16, opt.vram_O, opt.sd_version, opt.hf_key, opt.t_range,learned_embeds_path=opt.learned_embeds_path, scale=opt.att_scale, use_plus=opt.use_plus, use_clip=opt.use_clip, clip_t=opt.clip_t, clip_iterative=opt.clip_iterative, clip_version=opt.clip_version, image=opt.image,)
